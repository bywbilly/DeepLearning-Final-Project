...
class Tone_Classification():
    ...
    def build_model(self):
        ...
        split_x = tf.split(0, self.args['num_gpu'], x)
        split_y = tf.split(0, self.args['num_gpu'], y)
        ...
        for i in xrange(self.args['num_gpu']):
            with tf.device('/gpu:%d' % i):
                with tf.name_scope('sl_%d' % (i)) as scope:
                    pred, logits, _, _ = self._forward(split_x[i])
                    loss_on_tower = self._loss(logits, 0, 0, split_y[i])
        ...
        self.pred_step = tf.reshape(tower_preds, [self.args['batch_size'], 4])
